 ...
 * add two numbers
 *------------------------------------------------------------------*/
extern double ReduceAdd(double a, double b, YYLTYPE *bloc) { return a + b; }
/*--------------------------------------------------------------------
 * ReduceSub
 *
 * sub two numbers
 *------------------------------------------------------------------*/
extern double ReduceSub(double a, double b, YYLTYPE *bloc) { return a - b; }
/*--------------------------------------------------------------------
 * ReduceMult
 *
 * multiply two numbers
 *------------------------------------------------------------------*/
extern double ReduceMult(double a, double b, YYLTYPE *bloc) { return a * b; }
/*--------------------------------------------------------------------
 * ReduceDiv
 *
...
extern double ReduceDiv(double a, double b, YYLTYPE *bloc) {
  if (b == 0) {
    ...
    */
    ...
    /* */
    PrintError("division by zero! Line %d:c%d to %d:c%d", bloc->first_line,
               bloc->first_column, bloc->last_line, bloc->last_column);
    /* */
    return FLT_MAX;
  }
  return a / b;
...
extern void DumpVariables(char *prefix) {
  ...
  printf("%s Name------------------ Value----------\n", prefix);
  for (i = 0; i < nVars; i++)
    printf("%s '%-20.20s' %g\n", prefix, vars[i].name, vars[i].value);
  return;
}
/*--------------------------------------------------------------------
 * cmath.c
